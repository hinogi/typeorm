...
For example, to properly install the oracle driver you need to follow all instructions from
...
To run the tests, set up your environment configuration by copying `ormconfig.json.dist` into `ormconfig.json` and
...
**Executing only some tests**: When you are creating tests for some specific code, you may want only execute the tests that you're creating, so you waste less time verifying your code. To do this, you can temporarily modify your test definitions adding `.only` *mocha* commands **(describe, it)**. Example:
...
To run your tests, you need a DBMS installed on your machine. Alternatively, you can use docker
...
- The docker image of mssql-server requires at least 3.25 GB of RAM.
...
The instance will be run in the background,
...
It will start a Node.js instance which builds TypeORM and executes unit tests.
